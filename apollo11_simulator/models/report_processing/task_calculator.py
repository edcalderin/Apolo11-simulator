...
class TaskCalculator:
    ...
    def task_event_analysis(self) -> Tuple[str, pd.DataFrame]:
        """
                ...
                Returns:
                Tuple[str, pd.DataFrame]: A tuple containing a description and the resulting DataFrame.
            """

        result = self.__events.groupby(['mission', 'device_status'])
        ['device_type'].count().rename('Count').to_frame()
    ...
    def task_disconnection_management(self) -> Tuple[str, pd.DataFrame]:
        ...
        result = unkn_devices.groupby(['mission', 'device_type'])
        ['device_status'].count().rename('Count').to_frame()
    ...
    def task_devices_by_mission(self) -> Tuple[str, pd.DataFrame]:
        """
            Analyzes the number of devices per mission and calculates the percentage distribution.

            Returns:
                Tuple[str, pd.DataFrame]: A tuple containing a description and the resulting DataFrame.
            """
        devices_by_mission = self.__events.groupby(['mission', 'device_type'])
        ['device_type'].count().rename('Count')
        percentages = (devices_by_mission / devices_by_mission.groupby
        (level=0).sum()) * 100
    ...
    def task_mission_by_device_status(self) -> Tuple[str, pd.DataFrame]:
        """
            ...
            Returns:
                Tuple[str, pd.DataFrame]: A tuple containing a description and the resulting DataFrame.
            """
        mission_by_device_status_ = self.__events.groupby(['mission', 'device_status'])
        ['device_status']\
            .count().rename('count')
        result = (
                         mission_by_device_status_
                         / mission_by_device_status_.groupby(level=0)
                         .sum()
                 ) * 100
    ...
    def task_status_by_device_type_by_mission(self) -> Tuple[str, pd.DataFrame]:
        """
            Analyzes the number of device states based on device type and mission, including percentage distribution.
        ...
        percentages = (mission_by_devices_data
                       / mission_by_devices_data.groupby(level=0)
                       .sum()) * 100
        percentages = percentages.map(lambda x: f'{x:.2f} %')\
            .rename("percentage")\
            .to_frame()
        ...
        return "*** CANTIDAD DE ESTADOS POR DISPOSITIVO POR MISION ***",\
