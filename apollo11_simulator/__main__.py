...
if __name__ == '__main__':
    ...
    parser.add_argument('operation', choices=['generate-events',
                                              'generate-report'],
                        help='Tipo de operacion a realizar:'
                             ' generar eventos, generar reportes')
    ...
    match args.operation:
            case 'generate-events':
                ...
                event_manager = EventManager(input_data_file = event_params
                                             ["input_data_file"],
                                             target_path= event_params
                                             ["devices_path"],
                                             frequency_seconds= event_params
                                             ["frequency_seconds"],
                                             range_of_files= (event_params
                                                              ["range_of_files"]["min"],
                                                              event_params["range_of_files"]["max"]))
                event_manager()
            case 'generate-report':
                logger.info('Running in "generate report" mode')
                reporter = ReportBuilder.read_events(origin_path=event_params
                                                     ["devices_path"],
